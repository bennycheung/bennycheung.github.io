...
---
<!--excerpt.start-->
Our previous articles on [neural style transfer](https://bennycheung.github.io/deep-learning-on-windows-10) and [deep dream](https://bennycheung.github.io/deep-dream-on-windows-10) that was written 5 years ago, which was the first popular application of the AI technology on artistic works, but quickly made way for a more powerful and capable model named Textual Inversion. Stable Diffusion is a free tool using textual inversion technique for creating artwork using AI. The tool provides users with access to a large library of art generated by an AI model that was trained the huge set of images from ImageNet and the LAION dataset. The resulting Diffusion Models are able to create stunningly realistic images in a variety of mediums, including digital paintings, photo-based art, comics, and even animations. The tool is incredibly easy to use and can create images in seconds.
<!--excerpt.end-->
...
If you are interested in learning how to use Stable Diffusion to generate images from text prompts, read on!

After reading the following information page, you will be able to train a personal embeddings model for Stability Diffusion AI! Please note that there are no guarantees for performance, results, or safety of the program, the website using it, or the machine learning technology itself! Use at your own risk.
